...
Setattr=lambda obj,name,val:setattr(obj, name, val)
#Section 2:
...
#     can do this even if it is the name of one of the function's parameters.
    def __init__(self,*card,joker=False,faceup=False):
        ...
        if len(card)==0:
            ...
            card=list(card)
            if card[0] in self.RANKS:
                Setattr(self,'rank',self.RANKS.index(card[0]))
            elif card[0].lower=='joker':
                Setattr(self,'rank',13)
            else:
                card[0]=card[0][0].upper()
                if card[0] in self.RANKS:
                    Setattr(self,'rank',self.RANKS.index(card[0]))
                else:
                    raise 'error input' 
                    
            card[1]=card[1][0].upper()
            if card[1]=='S':
                Setattr(self,'suit',3)
            elif card[1]=='H':
                Setattr(self,'suit',2)

    ...
    for l in range(n):
        ...
        if len(moves) == 0:
            try:
                ...
                moves=[int(reduce(lambda x,y:x+y,map(str,(filter(None,map(lambda i:(B[i][0]=='  ')*(i+1),range(7))))))[rnd(len(reduce(lambda x,y:x+y,map(str,(filter(None,map(lambda i:(B[i][0]=='  ')*(i+1),range(7))))))))])]
            except:
                if "announce" in kws:
